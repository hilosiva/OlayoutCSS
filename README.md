...
デフォルトでは、左右に `5vw` の余白をつけてでボックスをセンタリングします。

なお、カスタムデータ属性を使うことで、デバイス幅によってのコンテンツ幅や最大値などを指定できます。

### data-offset - 余白量

左右の余白量を変更するには、`data-offset` 属性を用いて設定します。

なお、左側だけの余白を調整する `data-offset-left` 属性や、
右側だけの余白を調整する `data-offset-right` 属性もあります。
...
#### 全ての画面サイズで左右の余白を `10vw` にする例

```html
<div class="ol-container" data-offset="min:large">...</div>
```

#### 1024px 以上の画面サイズで左右の余白を `10vw` にする例

```html
<div class="ol-container" data-width="lg:large">...</div>
```

#### 0px 以上の画面サイズでは `2.5vw` 、768px 以上の画面幅では、`5vw`、1024px 以上の画面サイズでは `10vw` にする例
